...
// Redis toggle - same as verifyEmail
...
if (USE_REDIS) {
  try {
    ...
    console.log('🔴 Redis not available in register, falling back to memory storage');
  }
}
...
async function register(req, res, next) {
  ...
  try {
    ...
    // Check existing pending registration (Redis or Memory)
    ...
    if (USE_REDIS && redisUtils) {
      existingPendingRegistration = await redisUtils.getJSON(pendingRegistrationKey);
      ...
      if (global.pendingRegistrations && global.pendingRegistrations[email]) {
        existingPendingRegistration = global.pendingRegistrations[email];
        console.log(`[${startTimeFormatted}] - 🎯 Memory cache HIT for pending registration: ${email}`);
      }
    }
    ...
    // Store registration data with OTP (Redis or Memory)
    const pendingUser = {
      name,
      email,
    ...
    if (USE_REDIS && redisUtils) {
      await redisUtils.setJSON(pendingRegistrationKey, pendingUser, 900);
      ...
      console.log(`[${startTimeFormatted}] - 💾 Pending registration stored in memory: ${email}`);
      usedRedis = false;
    }

    // Generate temporary token for email verification only
    console.log(`[${startTimeFormatted}] - 🔍 JWT_SECRET during registration: ${process.env.JWT_SECRET}`);
    ...
    console.log(`[${startTimeFormatted}] - 🔍 GENERATED TEMP TOKEN: ${tempToken}`);
    console.log(`[${startTimeFormatted}] - 🔍 TOKEN PAYLOAD:`, JSON.stringify({
      email: email,
      type: 'email_verification',
      temp: true
    }, null, 2));
