  ...
  async handleWebhook(req, res) {
    ...
    try {
      ...
      if (eventType === 'payment.paid') {
        ...
        // Log payment metadata if available
        if (payment?.attributes?.metadata) {
          console.log(`[${formatDate()}] - 📋 PAYMENT METADATA:`, JSON.stringify(payment.attributes.metadata, null, 2));
          /**
           * use this to update payment status in your database
           * Send email to user
           */
        }
        
        // Log payment intent metadata if available
        if (payment?.attributes?.payment_intent?.metadata) {
          console.log(`[${formatDate()}] - 📋 PAYMENT INTENT METADATA:`, JSON.stringify(payment.attributes.payment_intent.metadata, null, 2));
        }
        ...
        // Log link payment metadata if available
        if (link?.attributes?.metadata) {
          console.log(`[${formatDate()}] - 📋 LINK PAYMENT METADATA:`, JSON.stringify(link.attributes.metadata, null, 2));
        }
      }
      ...
      if (event?.data?.attributes?.metadata) {
        console.log(`[${formatDate()}] - 📋 EVENT METADATA:`, JSON.stringify(event.data.attributes.metadata, null, 2));
      }
